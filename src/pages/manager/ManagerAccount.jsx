...
const ManagerAccount = () => {
  ...
  const { staff, isLoading, pagination, fetchMyStoreStaff, clearError } =
    useManagerStore();
  ...
  const loadUsersInit = async (page = 1, limit = itemsPerPage) => {
    try {
      ...
      const result = await fetchMyStoreStaff(params);
      setStats(
        result.data.stats || {
          totalStaff: 0,
          activeStaff: 0,
      ...
      if (result.data && result.data.staff) {
        Notification.success(
          `Tải thành công ${result.data.staff.length} tài khoản.`
        );
      }
    } catch (error) {
      Notification.error(
  ...
  const loadUsers = async (page = 1, limit = itemsPerPage) => {
    try {
      ...
      const result = await fetchMyStoreStaff(params); // Gọi API để lấy danh sách người dùng
      const fetchStats = result.data.stats || {
        totalStaff: result.data.stats?.totalStaff || 0,
        activeStaff: result.data.stats?.activeStaff || 0,
        inactiveStaff: result.data.stats?.inactiveStaff || 0,
      };
      setStats(fetchStats);
    } catch (error) {
      Notification.error(
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-3xl font-bold text-green-900">
                    {stats?.activeStaff?.toLocaleString() ?? 0}
                  </p>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-3xl font-bold text-red-900">
                    {stats?.inactiveStaff?.toLocaleString() ?? 0}
                  </p>
            ...
            <div className="flex justify-between items-center mt-4 flex-col border-t border-gray-200 px-5 py-4 sm:flex-row">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="mb-4 text-sm text-gray-600 font-semibold flex items-center">
                  Hiển thị {staff.length} / {pagination.totalUsers} người dùng
