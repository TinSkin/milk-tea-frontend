...
mergeDuplicateItems: () => {
  ...
  console.log("🔄 [CartStore] Starting merge process with items:", items.length);
...
forceMergeAndDebug: () => {
  ...
  return mergedItems;
      ...
      _transformBackendToLocal: (backendItems) => {
        ...
        console.log("🔄 [CartStore] Transforming backend items:", backendItems.length);
...
get()._log("✅ [CartStore] Synced _id after merge:", syncedItems.length);
return syncedItems;
    ...
    throw error;
      ...
      updateCartItem: async (oldItem, updatedItem) => {
        ...
        try {
          ...
          console.log("📤 [CartStore] Gửi cập nhật đến backend:", JSON.stringify(updatePayload, null, 2));
          ...
          console.error("❌ [CartStore] LỖI cập nhật item:", error.response?.data || error.message || error);
          throw error;
        }
      },
